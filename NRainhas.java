...
public class NRainhas {
	...
	/**
	 * Seleciona o algoritmo e execução, só é chamando por um evento de um botão
	 * da interface
	 * 
	 * @param void
	 * @return void
	...
	public void executa(String tipoAlgoritmo) throws Exception {
		...
		case Sequencial:
			/*
			 * Instância uma thread, ou seja, se torna sequencial
			 */
			queen = new paraleloNRainhas(0, rainhas, rainhas, nomeArquivo);
			queen.start();
			break;

		case Paralelo:
			/*
			 * divide a quantidade de rainhas entre as threads cria N
			 * (quantidade de processadores disponiveis) threads e executa
			 */
			escalonarTarefas();
			iniciarTarefas();
			break;
	...
	/**
	 * Gera um vetor contendo o número de linhas que cada thread devera tentar
	 * econtrar
	 * 
	 * @param void
	 * @return void
	...
	private void iniciarTarefas() throws IOException {
		for (int i = 0; i < processadores; i++) {
			...
			queen = new paraleloNRainhas(inferior, superior, rainhas,
					nomeArquivo);
			queen.start();
			inferior = superior;
		}
	...
	/**
	 * Set para a quantidade de rainhas a serem calculadas
	 * 
	 * @param int rainhas
	 * @return void
	 */
